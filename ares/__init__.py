...
class _RDAProxy:
    ...
    def __call__(self, x, sigma, **extra):
        ...
        if rel < thr:
            ...
            yr = _rms(y)
            if yr > 0 and r > 0:
                s = torch.clamp(r / yr, max=2.0).to(y)
                y = y * s
            self.prev2 = self.prev
            self.prev = y
...
def sample_ares_rda(model, x, sigmas, extra_args=None, callback=None, disable=False,
    ...
    n = int(sigmas.numel()) if torch.is_tensor(sigmas) else len(sigmas)
    if (_ARES_STEP is None) or (n < 2):
        return sample_ares(model, x, sigmas, extra_args=extra_args, callback=callback, disable=disable,
                           c2=c2, simple_phi_calc=simple_phi_calc, momentum=momentum,
