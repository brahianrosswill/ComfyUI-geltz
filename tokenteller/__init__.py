...
import math, typing as T, torch, torch.nn.functional as F

def _looks_like_tokens(t): return (isinstance(t, torch.Tensor) and t.dim()==3 and t.shape[1]>=1 and t.shape[2]>=64)
def _gather_3d(obj):
    o=[]
    if isinstance(obj, torch.Tensor):
        if _looks_like_tokens(obj): o.append(obj)
    elif isinstance(obj,(list,tuple)):
        for v in obj: o+=_gather_3d(v)
    elif isinstance(obj,dict):
        for v in obj.values(): o+=_gather_3d(v)
    return o
def _gather_texts(cond):
    cands=[]
    if isinstance(cond,dict):
        for k in ("prompt","text","token_strings","tokens","caption","string"):
            if k in cond: cands.append(cond[k])
    return cands
def _to_words(x):
    if isinstance(x,str): return x.split()
    if isinstance(x,(list,tuple)) and all(isinstance(v,str) for v in x): return list(x)
    return None
def conditioning_to_tokens_and_words(conditioning: T.List, limit_streams:int=4):
    streams=[]; words=[]
    for pair in conditioning:
        if not isinstance(pair,(list,tuple)) or len(pair)==0: continue
        cond=pair[0]
        ...
        for t in cands:
            ...
            if len(streams)>=limit_streams: break
        if len(words)==0:
    ...
    return tok,words
def token_values(tokens: torch.Tensor, mode="norm"):
    if tokens.dim()!=2 or tokens.numel()==0: return torch.zeros(0)
    if mode=="norm": v=torch.linalg.vector_norm(tokens,dim=1)
    elif mode=="var": v=tokens.var(dim=1)
    elif mode=="mean": v=tokens.mean(dim=1).abs()
    else: v=torch.linalg.vector_norm(tokens,dim=1)
...
def _raster_curve(h,w,pts,thick):
    ...
    if yx.shape[0]==0: return img
    n=yx.shape[0]
    for i in range(n):
        y,x=yx[i]
        y=float(y); x=float(x)
        y0=max(0,int(round(y-3*dev))); y1=min(h-1,int(round(y+3*dev)))
        x0=max(0,int(round(x-3*dev))); x1=min(w-1,int(round(x+3*dev)))
...
def _draw_char(img,y,x,scale,ch,val=1):
    ...
    for r,row in enumerate(g):
        for c in range(5):
            if (row>>(4-c))&1:
                yy=y+r*scale; xx=x+c*scale
                if yy>=H or xx>=W: continue
...
def _apply_spikes(pts,vals,spike_h):
    if len(vals)==0 or pts.shape[0]==0: return pts
    v=vals.detach().float()
    v=(v-v.min())/(v.max()-v.min()+1e-6)
    yx=pts.clone()
    yx[:,0]-=v*spike_h
    return yx
def _smooth_path(yx,samples_per_seg=20):
    n=yx.shape[0]
    if n<2: return yx
    pts=[]
    for i in range(n-1):
...
class tokenteller:
    ...
    RETURN_NAMES=("image",)
    OUTPUT_NODE=False
    def render(self,conditioning,out_res=768,limit_streams=4,amp=80.0,phase=0.0,thickness=3.0,spike_height=140.0,text_scale=2,spacing=2,value_mode="norm"):
        ...
        for i in range(n_words):
            ...
            y_cursor+=line_h
            if y_cursor+8*text_scale>=h: break
        ...
        return (rgb,)
tt=tokenteller
NODE_CLASS_MAPPINGS={"tokenteller":tokenteller}
NODE_DISPLAY_NAME_MAPPINGS={"tokenteller":"tokenteller"}
