...
class VelocityScaling:
    ...
    def INPUT_TYPES(s):
        return {"required": {
            "model": ("MODEL",),
            "strength": ("FLOAT", {"default": 1.005, "min": 0.9, "max": 1.2, "step": 0.001}),
        }}
    ...
    def apply(self, model, strength):
        ...
        def vs_out(x, extra_options):
            mt = None
            if isinstance(extra_options, dict):
                mt = extra_options.get("model_type") or extra_options.get("prediction_type")
                s = extra_options.get("sampling")
                if mt is None and isinstance(s, dict):
                    mt = s.get("prediction_type")
            if isinstance(mt, str) and mt.lower().startswith("v"):
                return x / strength
            return x

        if hasattr(m, "set_model_output_patch"):
            m.set_model_output_patch(vs_out)
        else:
            opts = dict(getattr(m, "model_options", {}))
            opts["model_output_patch"] = vs_out
